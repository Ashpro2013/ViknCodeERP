    ...
    public class PurchaseOrderRepo : IPurchaseOrder
    {
        ...
        public bool Create(PoHeader poHeader)
        {
            ...
            try
            {
                ...
                List<StockTranLog> stockTranLogList = new List<StockTranLog>();
                foreach (PoDetail item in poHeader.PoDetails)
                {
                    var stock = new StockTranLog
                    {
                        Id = 1,
                        Date = poHeader.PoDate,
                        PoHeaderId = 1,
                        ProductId = item.ProductCode.ToInt32(),
                        Quantity = item.Quantity,
                        UnitId = _context.Units.FirstOrDefault(x => x.Name == item.UnitName).Id
                    };
                    stockTranLogList.Add(stock);
                }
                _context.StockTranLogs.AddRange(stockTranLogList);
        ...
        public bool Delete(PoHeader poHeader)
        {
            ...
            try
            {
                ...
                List<StockTranLog> stockTranLogs = _context.StockTranLogs.Where(x => x.PoHeaderId == poHeader.Id).ToList();
                _context.StockTranLogs.RemoveRange(stockTranLogs);
